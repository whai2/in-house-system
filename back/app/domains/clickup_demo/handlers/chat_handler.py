...
from app.domains.clickup_demo.models.documents import SessionDocument, ChatDocument
...
class ChatHandler:
    ...
    async def save_chat_from_stream_event(
        ...
        node_sequence: List[str],
        execution_logs: List[Dict[str, Any]],
        tool_details: List[Any],  # ToolExecutionDetail objects
    ) -> str:
        """
        스트리밍 이벤트에서 채팅 저장
        ...
        Args:
            ...
            node_sequence: 실행된 노드 순서
            execution_logs: 실행 로그
            tool_details: 도구 실행 상세 정보 (ToolExecutionDetail 객체 리스트)
